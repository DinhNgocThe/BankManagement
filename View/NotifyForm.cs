    ...
    public partial class NotifyForm : Form
    {
        ...
        public void updateFlowPannel()
        {
            try
            {
                ...
                // Tạm dừng Layout để cải thiện hiệu suất
                flowPanelNotifyForm.SuspendLayout();

                foreach (DataRow row in viewModel.NotifyTable.Rows)
                {
                    // Chuyển đổi "DateCreated" thành DateTime
                    DateTime time = DateTime.TryParse(row["DATECREATED"]?.ToString(), out DateTime parsedTime) ? parsedTime : DateTime.MinValue;
                    string formattedTime = time == DateTime.MinValue ? "N/A" : time.ToString("dd/MM/yyyy HH:mm");

                    // Lấy các giá trị khác, đảm bảo không lỗi null
                    int notification_id = Convert.ToInt32(row["NOTIFICATION_ID"] ?? 0);
                    int staff_id = Convert.ToInt32(row["STAFF_ID"] ?? 0);
                    string title = row["TITLE"]?.ToString() ?? "No Title";
                    string message = row["MESSAGE"]?.ToString() ?? "No Message";
                    int status = Convert.ToInt32(row["ISREAD"] ?? 0);

                    // Thêm mục vào giao diện
                    AddItem(staff_id, notification_id, title + "\r\n" + formattedTime, message, status);
                }

                // Tiếp tục Layout
                flowPanelNotifyForm.ResumeLayout();
            }
            catch (Exception ex)
            {
                // Xử lý ngoại lệ
                CustomMessageBox.ShowBox("Error when updating FlowPanel: " + ex.Message, "Error");
            }
        }
