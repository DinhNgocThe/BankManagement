...
namespace BankManagement
{
    public partial class MainForm : Form
    {
        ...
        void ChangeLanguage()
        {
            ...
            lbAdministrator.Text = LangHelper.Instance.GetString("Administrator");
            btnAddStaff.Text = LangHelper.Instance.GetString("Add Staff");
            btnCreateNotification.Text = LangHelper.Instance.GetString("Create Notification");
        }
		...
		private void btnCustomer_Click(object sender, EventArgs e)
		{
            ...
            btnAddStaff.FillColor = initialButtonColor;
            btnCreateNotification.FillColor = initialButtonColor;
            ...
            if (createNotificationForm != null && !createNotificationForm.IsDisposed)
            {
                createNotificationForm.Close();
            }
        ...
        private void btnLoan_Click(object sender, EventArgs e)
        {
            ...
            btnAccount.FillColor = initialButtonColor;
            btnTransaction.FillColor = initialButtonColor;
            btnLoan.FillColor = clickedButtonColor;
            btnAddStaff.FillColor = initialButtonColor;
            btnCreateNotification.FillColor = initialButtonColor; //Đổi màu khi click, set màu các btn khác về ban đầu
        ...
        //Sự kiện click btn Add Staff---------------------------------------------------------------------------------------------------------------------------------------------------
        private AddStaffForm addStaffForm;
        private void btnAddStaff_Click(object sender, EventArgs e)
        {
            ...
            btnTransaction.FillColor = initialButtonColor;
            btnLoan.FillColor = initialButtonColor;
            btnAddStaff.FillColor = clickedButtonColor;
            btnCreateNotification.FillColor = initialButtonColor;//Đổi màu khi click, set màu các btn khác về ban đầu

            //Mở form AddStaffForm
            if (addStaffForm == null || addStaffForm.IsDisposed) // Kiểm tra nếu form chưa được khởi tạo hoặc đã bị đóng
            {
                addStaffForm = new AddStaffForm(staffId);
                addStaffForm.StartPosition = FormStartPosition.Manual;
                addStaffForm.Height = this.Height - 63; //Chỉnh độ cao của form CustomerAccountForm
                addStaffForm.Width = this.Width - panelLeftBarMain.Width - 3;
                addStaffForm.Location = new Point(this.Location.X + panelLeftBarMain.Width + (this.Width - panelLeftBarMain.Width - addStaffForm.Width) / 2, this.Location.Y + 60);
                addStaffForm.Show(this);
            }
            else
            {
                // Nếu form đã mở, chỉ cần kích hoạt và đưa nó lên trên cùng
                addStaffForm.BringToFront();
                addStaffForm.Activate();
            }
        ...
        //Sự kiện click btn Create Notification---------------------------------------------------------------------------------------------------------------------------------------------------
        private CreateNotificationForm createNotificationForm;
        private void btnCreateNotification_Click(object sender, EventArgs e)
        {
            ...
            btnLoan.FillColor = initialButtonColor;
            btnAddStaff.FillColor = initialButtonColor;
            btnCreateNotification.FillColor = clickedButtonColor;//Đổi màu khi click, set màu các btn khác về ban đầu

            //Mở form AddStaffForm
            if (createNotificationForm == null || createNotificationForm.IsDisposed) // Kiểm tra nếu form chưa được khởi tạo hoặc đã bị đóng
            {
                createNotificationForm = new CreateNotificationForm(staffId);
                createNotificationForm.StartPosition = FormStartPosition.Manual;
                createNotificationForm.Height = this.Height - 63; //Chỉnh độ cao của form CustomerAccountForm
                createNotificationForm.Width = this.Width - panelLeftBarMain.Width - 3;
                createNotificationForm.Location = new Point(this.Location.X + panelLeftBarMain.Width + (this.Width - panelLeftBarMain.Width - createNotificationForm.Width) / 2, this.Location.Y + 60);
                createNotificationForm.Show(this);
            }
            else
            {
                // Nếu form đã mở, chỉ cần kích hoạt và đưa nó lên trên cùng
                createNotificationForm.BringToFront();
                createNotificationForm.Activate();
            }
        ...
        private void Main_LocationChanged(object sender, EventArgs e) //Cập nhật form con khi di chuyển form
		{
            ...
            UpdateAddStaffFormSizeAndPosition();
            UpdateCreateNotificationFormSizeAndPosition();
        }
        ...
        //Hàm cập nhật trạng thái AddStaffForm
        private void UpdateAddStaffFormSizeAndPosition()
        {
            if (addStaffForm != null && !addStaffForm.IsDisposed)
            {
                // Đặt vị trí của Form con và chỉnh size
                addStaffForm.Height = this.Height - 63; //Chỉnh độ cao của form CustomerForm
                addStaffForm.Width = this.Width - panelLeftBarMain.Width - 3;
                addStaffForm.Location = new Point(this.Location.X + panelLeftBarMain.Width + (this.Width - panelLeftBarMain.Width - addStaffForm.Width) / 2, this.Location.Y + 60);
            }
        }
        ...
        //Hàm cập nhật trạng thái AddStaffForm
        private void UpdateCreateNotificationFormSizeAndPosition()
        {
            if (createNotificationForm != null && !createNotificationForm.IsDisposed)
            {
                // Đặt vị trí của Form con và chỉnh size
                createNotificationForm.Height = this.Height - 63; //Chỉnh độ cao của form CustomerForm
                createNotificationForm.Width = this.Width - panelLeftBarMain.Width - 3;
                createNotificationForm.Location = new Point(this.Location.X + panelLeftBarMain.Width + (this.Width - panelLeftBarMain.Width - createNotificationForm.Width) / 2, this.Location.Y + 60);
            }
        }
